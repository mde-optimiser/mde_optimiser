...
public class CreateNodeRuleCommand implements IRuleGenerationCommand {
	...
	public CreateNodeRuleCommand(MetamodelWrapper metamodelWrapper, RuleSpec ruleSpec, List<RepairSpec> repairSpecs) {
		this.metamodelWrapper = metamodelWrapper;
		this.ruleSpec = ruleSpec;
		this.repairSpecs = repairSpecs;
	}
	
	public CreateNodeRuleCommand(EClass node, MetamodelWrapper metamodelWrapper, RuleSpec ruleSpec, List<RepairSpec> repairSpecs){
		this.metamodelWrapper = metamodelWrapper;
		this.node = node; 
		this.repairSpecs = repairSpecs;
		this.ruleSpec = ruleSpec;
	...
	public Module generate() {
      ...
      var context = classifierInfo.get(classifierInfo.keySet().toArray()[contextReferenceId]);
        ...
        Rule rule = null;
        try {
          rule = Common.createBasicRule(module, (EReference) classifierInfo.keySet().toArray()[contextReferenceId],
        ...
        var newNode = HenshinRuleAnalysisUtilEx.getRHSMinusLHSNodes(rule).get(0);
          ...
          if(childInfo.hasMandatories()) {
            try {
              Common.createMandatoryChildren(rule, childInfo, newNode, OperationType.CREATE,
                      true, metamodelWrapper.getMetamodelAnalyser(), elementFilter);
            } catch (OperationTypeNotImplementedException e) {
              e.printStackTrace();
            }
            try {
              Common.createMandatoryNeighbours(rule, childInfo, newNode,
                      OperationType.CREATE, true, elementFilter);
            } catch (OperationTypeNotImplementedException e) {
              e.printStackTrace();
            }
          }
	...
	private void applyRepairOperations(Rule rule, RuleSpec ruleSpec, List<RepairSpec> repairSpecs){
	...
	private void applyRepairOperationsLbMany(Rule rule, List<RepairSpec> repairSpecs){
		...
		repairSpecs.stream().filter(repairSpec -> repairSpec.getRepairSpecType() == RepairSpecType.CREATE_LB_REPAIR_MANY
      ...
      if (repairSpec.getRepairSpecType() == RepairSpecType.DELETE_LB_REPAIR_MANY) {
        ...
        if (repairSpec.getEdge().getEOpposite().getLowerBound() == repairSpec.getEdge().getEOpposite().getUpperBound()
          ...
          try {
            Common.createRepairOppositeMandatoryNeighbours(rule, childInfo, createdNode, OperationType.CREATE,
                    true, metamodelWrapper.getMetamodelAnalyser(), elementFilter);
          } catch (OperationTypeNotImplementedException e) {
            e.printStackTrace();
          }
      ...
      createdNode.getOutgoing(repairSpec.getEdge()).forEach(outgoingEdge -> {
        ...
        var existingSourceNode = HenshinRuleAnalysisUtilEx.createPreservedNode(rule, existingsourceNodeName, this.getNode());
	...
	private void applyRepairOperationsLbSingle(Rule rule, List<RepairSpec> repairSpecs){
			...
			repairSpecs.stream().filter(repairSpec -> repairSpec.getRepairSpecType() == RepairSpecType.DELETE_LB_REPAIR
