...
public class RuleGenerationCommandFactory {
  ...
  public List<IRuleGenerationCommand> makeCommand(String repairSpecsType, MetamodelWrapper metamodelWrapper, RuleSpec ruleSpec, List<RepairSpec> repairSpecs) {
    ...
    switch(repairSpecsType) {
      case "CREATE":
        ruleGenerationCommands.add(new CreateNodeRuleCommand(metamodelWrapper, ruleSpec, repairSpecs)); break;
      case "DELETE":
        ruleGenerationCommands.add(new DeleteNodeRuleCommand(metamodelWrapper, ruleSpec, repairSpecs)); break;
      //Must be Add / Remove edge cases
      default: {
        ruleGenerationCommands.addAll(repairSpecs.stream().map(repairSpec -> makeEdgeCommand(metamodelWrapper, ruleSpec, repairSpec)).collect(Collectors.toList()));
      }
    }
  ...
  public IRuleGenerationCommand makeEdgeCommand(MetamodelWrapper metamodelWrapper, RuleSpec ruleSpec, RepairSpec repairSpec) {
