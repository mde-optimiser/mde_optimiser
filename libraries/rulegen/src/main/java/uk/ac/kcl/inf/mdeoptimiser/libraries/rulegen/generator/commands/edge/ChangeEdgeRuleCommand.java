...
public class ChangeEdgeRuleCommand implements IRuleGenerationCommand {
  ...
  public ChangeEdgeRuleCommand(
      EClass node, MetamodelWrapper metamodelWrapper, RuleSpec ruleSpec, RepairSpec repairSpec) {
    this.metamodelWrapper = metamodelWrapper;
    this.node = node;
    this.repairSpec = repairSpec;
    this.ruleSpec = ruleSpec;
  ...
  public Module generate() {
    ...
    // Set module name
    var ruleName =
        RuleNameGenerator.getRuleName(ruleSpec, repairSpec, this.metamodelWrapper.ruleType());
