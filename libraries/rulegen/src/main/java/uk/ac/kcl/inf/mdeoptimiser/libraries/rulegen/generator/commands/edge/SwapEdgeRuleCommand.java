...
public class SwapEdgeRuleCommand implements IRuleGenerationCommand {
	...
	public SwapEdgeRuleCommand(MetamodelWrapper metamodelWrapper, RuleSpec ruleSpec, RepairSpec repairSpecs) {
		this.metamodelWrapper = metamodelWrapper;
		this.ruleSpec = ruleSpec;
		this.repairSpecs = repairSpecs;
	}
	
	public SwapEdgeRuleCommand(EClass node, MetamodelWrapper metamodelWrapper, RuleSpec ruleSpec, RepairSpec repairSpecs){
		this.metamodelWrapper = metamodelWrapper;
		this.node = node; 
		this.repairSpecs = repairSpecs;
		this.ruleSpec = ruleSpec;
	...
	public Module generate() {
      ...
      var context = classifierInfo.get(classifierInfo.keySet().toArray()[contextReferenceId]);
			...
			for(var contextId = 0; contextId < context.size(); contextId++){
				...
				var rule = generateBasicRule(module, this.getEdge(), this.getNode(), (EClass) this.getEdge().getEType());
	...
	private Rule generateBasicRule(Module module, EReference reference, EClass sourceNode, EClass targetNode){
