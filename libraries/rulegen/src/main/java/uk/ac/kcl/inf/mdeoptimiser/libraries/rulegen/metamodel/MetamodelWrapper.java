...
public class MetamodelWrapper {
  ...
  private void setMetamodel(EPackage metamodel, List<Multiplicity> multiplicityRefinements) {
    ...
    multiplicityRefinements.forEach(
        multiplicity -> {
  ...
  /**
   * Because some metamodels are loaded as singletons, we have to revert changes
   *
   * @param multiplicity
   */
  public void revertEdgeMultiplicities(Multiplicity multiplicity) {
    var edge = getEdge(multiplicity);
    edge.setLowerBound(multiplicity.getProblemLower());
    edge.setUpperBound(multiplicity.getProblemUpper());
  }
  ...
  private void updateEdgeLowerBound(Multiplicity multiplicity) {
    ...
    if (edge.getLowerBound() <= multiplicity.lower) {
      multiplicity.setProblemLower(edge.getLowerBound());
  ...
  private void updateEdgeUpperBound(Multiplicity multiplicity) {
    ...
    if ((edge.getUpperBound() == -1 && multiplicity.upper >= edge.getUpperBound())
      ...
      multiplicity.setProblemUpper(edge.getUpperBound());
  ...
  public EClass getNode(String nodeName) {
    ...
    return (EClass)
        this.getMetamodel().getEClassifiers().stream()
            .filter(classifier -> classifier.getName().equals(nodeName))
            .findFirst()
            .get();
  }
