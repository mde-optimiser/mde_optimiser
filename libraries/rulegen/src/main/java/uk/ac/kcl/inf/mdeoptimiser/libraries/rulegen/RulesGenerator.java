...
public class RulesGenerator {
	...
	public RulesGenerator(EPackage metamodel, List<Multiplicity> refinedMultiplicities, List<RuleSpec> rulegenSpecs){
	...
	public Map<EPackage, List<Module>> generateRules(){
      ...
      generatedRulesListProblem.addAll(this.generateSpecRules(ruleSpec, repairSpecs, problemMetamodelWrapper));
		...
		if(this.refinedMultiplicities.size() > 0) {
			var solutionMetamodelWrapper = new MetamodelWrapper(this.metamodel, this.refinedMultiplicities);
	...
	private List<Module> removeDupplicateRules(List<Module> problemRules, List<Module> solutionRules){
	...
	private List<Module> generateSpecRules(RuleSpec ruleSpec, Map<String, Set<List<RepairSpec>>> mappedRepairSpecs, MetamodelWrapper metamodelWrapper) {
