...
public class MetamodelGenerator {
  ...
  public EReference getEdge(String edgeSource, String edgeName){
    var container = (EClass) metamodel.getEClassifiers()
            .stream()
            .filter(classifier -> classifier.getName().equals(edgeSource)).findFirst().get();

    var references = container.getEAllStructuralFeatures()
            .stream()
            .filter( reference -> reference.getName().equals(edgeName)).collect(Collectors.toList());
