...
/**
 * An extended version of the condition handler that allows to specify which scripting engine
 * handles which protocol (specified in the attribute conditions).
 */
public class MomotEngine extends EngineImpl implements Engine {
  ...
  public Iterable<Match> findMatches(
      final Rule rule, final EGraph graph, final Match partialMatch) {
    updateOCLEngine(graph);
  ...
  protected void updateOCLEngine(final EGraph graph) {
    ...
    getDispatcherWrapper()
        .getEngine(OCLScriptEngine.PROTOCOL_PREFIX)
