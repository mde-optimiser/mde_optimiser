...
public class CombinationProvider {
	...
	/**
	 * Increment "node2pointer". Finds the first element that can be incremented
	 * (starting from the end, proceeding in reverse order). Increments this element
	 * and sets the indexes of subsequent elements to zero.
	 * 
	 * @return True if the incrementing succeeded, false if it was not possible
	 */
	public static boolean countOneUp(MatchingInfo bindingInfo, LinkedHashMap<Node, Integer> node2pointer) {
	 ...
	 * 
	 * @return True if a new random assignment was set, false if that was not
	 *         possible
	 */
	public static boolean getRandomAssignment(MatchingInfo bindingInfo, LinkedHashMap<Node, Integer> node2pointer,
		...
		while (usedCombinations.size() < possibleCombinations) {
			for (Entry<Node, Integer> entry : node2pointer.entrySet()) {
				entry.setValue(
				getRandomPosition(bindingInfo, entry.getKey()));				
			}
