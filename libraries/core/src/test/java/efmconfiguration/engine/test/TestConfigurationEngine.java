...
/**
 * Tests the engine with the default setting (only one match is found).
 * TestConfigurationEngineAllMatches tests under the setting that all possible matches are to be
 * found.
 *
...
public class TestConfigurationEngine {
  ...
  public void testActSMSTransfer() {
    HenshinResourceSet resourceSet =
        new HenshinResourceSet("src/test/java/efmconfiguration/engine/test");
    ...
    Change change =
        engine.createChange(ruleActSMSTransfer, graph, match, new MatchImpl(ruleActSMSTransfer));
  ...
  public void testDeScreen3Applicable() {
    ...
    assertTrue(change instanceof CompoundChange);
    assertEquals(4, ((CompoundChange) change).getChanges().size());
    //		((CompoundChange) change).getChanges().forEach(c -> System.out.println(((AttributeChange)
    // c).getObject() + " " + ((AttributeChange) c).getNewValue()));
