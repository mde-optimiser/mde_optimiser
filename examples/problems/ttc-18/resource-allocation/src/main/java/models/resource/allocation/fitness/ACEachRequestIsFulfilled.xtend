...
class ACEachRequestIsFulfilled implements IGuidanceFunction {
	...
	override computeFitness(EObject solution) {
		...
		try {
			...
			val constraint = SolutionInterpreter.checkThatAllRequestsAreFulfilled(model.solution)
	...
	override getName() {
		return "Architectural constraint: Check that all requests are fulfilled"
	}
}