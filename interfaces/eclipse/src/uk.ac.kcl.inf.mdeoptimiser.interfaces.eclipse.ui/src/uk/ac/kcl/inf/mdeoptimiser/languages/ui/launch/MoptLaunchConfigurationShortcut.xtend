...
class MoptLaunchConfigurationShortcut implements ILaunchShortcut {
	...
	def void launch(IFile moptFile, String mode){
		...
		val launchConfigurationType = launchManager
				.getLaunchConfigurationType(MoptLaunchConfigurationAttributes.MDEO_LAUNCH_CONFIGURATION_TYPE)
		
		try {
			val launchConfiguration = launchManager.getLaunchConfigurations(launchConfigurationType).filter[ l | 
				l.getAttribute(MoptLaunchConfigurationAttributes.ATTR_MOPT_SOURCE_PATH, "").equals(moptFilePath)
			].head
