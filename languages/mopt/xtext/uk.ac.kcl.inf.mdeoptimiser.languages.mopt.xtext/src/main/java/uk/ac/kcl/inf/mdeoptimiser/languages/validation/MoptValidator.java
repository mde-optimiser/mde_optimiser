...
public class MoptValidator extends AbstractMoptValidator {
  ...
  @Check
  public void checkTerminationSpecHasNoDuplicateParameters(
      TerminationConditionSpec terminationCondition) {
  ...
  public void checkAlgorithmHasAllRequiredParameters(AlgorithmSpec algorithm) {
    ...
    var requiredParameters =
        AlgorithmParametersConfiguration.getInstance().get(algorithm).stream()
            .filter(parameter -> parameter.isRequired());

    requiredParameters.forEach(
        requiredParameter -> {
          if (!algorithm.getParameters().stream()
              .anyMatch(
                  algorithmParameter ->
                      algorithmParameter.getName().equals(requiredParameter.getName()))) {
            error(
                String.format(
                    "Required parameter '%s' must be specified.", requiredParameter.getName()),
                null,
  ...
  public void checkAlgorithmHasNoRedundantParameters(AlgorithmSpec algorithm) {
    ...
    algorithm
        ...
        .forEach(
            algoParameter -> {
              if (!expectedParameters.stream()
                  .anyMatch(
                      algorithmParameter ->
                          algorithmParameter.getName().equals(algoParameter.getName()))) {
                warning(
                    String.format(
                        "Unexpected parameter '%s' will be ignored.", algoParameter.getName()),
                    null,
