...
class MoptValidator extends AbstractMoptValidator {
		...
		dupParameters.forEach [ p1, p2 |
			error(
				String.format(
					...
					p1, p2.length), algorithm, null, MoptValidatorIssues.DUPLICATE_PARAMETER_ENCOUNTERED);
		]
	}
	...
	def void checkTerminationSpecHasNoDuplicateParameters(TerminationConditionSpec terminationCondition) {
		...
		val dupParameters = terminationCondition.parameters.groupBy[parameter|parameter.name].filter [ key, value |
			value.length > 1
		];
		...
		dupParameters.forEach [ p1, p2 |
			error(
				String.format(
					"Duplicate parameter found. Expecting '%s' to be specified only once. Specified '%s' times instead",
					p1, p2.length), terminationCondition, null, MoptValidatorIssues.DUPLICATE_PARAMETER_ENCOUNTERED);
		]
	}
	...
	def void checkAlgorithmHasAllRequiredParameters(AlgorithmSpec algorithm) {
		...
		val requiredParameters = AlgorithmParametersConfiguration.getInstance().get(algorithm).filter [ parameter |
			parameter.required == true
		]

		requiredParameters.forEach [ requiredParameter |

			if (algorithm.parameters.filter[algorithmParameter|algorithmParameter.name.equals(requiredParameter.name)].
				empty) {
				error(String.format("Required parameter '%s' must be specified.", requiredParameter.name), null,
	...
	def void checkAlgorithmHasNoRedundantParameters(AlgorithmSpec algorithm) {
		...
		algorithm.parameters.forEach [ algoParameter |

			if (expectedParameters.filter[algorithmParameter|algorithmParameter.name.equals(algoParameter.name)].
				empty) {
				warning(String.format("Unexpected parameter '%s' will be ignored.", algoParameter.name), null,
